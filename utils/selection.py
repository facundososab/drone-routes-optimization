...
def tournament_selection(pop, fitnesses, k):
    ...
    mejor_idx = fitness_candidatos.index(min(fitness_candidatos))
...
def buscar_n_mejores(poblacion, fitness_scores, N_BEST):
    ...
    sorted_population = [x for _, x in sorted(zip(fitness_scores, poblacion), key=lambda pair: pair[0])]  #De menor a mayor
    return sorted_population[:N_MEJORES] # Devuelve los N_mejores fitness (serian los minimos)
